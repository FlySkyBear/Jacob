...
import com.jacob.com.util.OSInfo;
...
public class JacobDllLoader {
    ...
    static void cleanup() {
        ...
        File[] nativeLibFiles = dir.listFiles(new FilenameFilter() {
            private final String searchPattern = "jacob-" + getVersion();
    ...
    private static boolean extractAndLoadLibraryFile(String libFolderForCurrentOS, String libraryFileName,
        ...
        String extractedLibFileName = String.format("jacob-%s-%s-%s", getVersion(), uuid, libraryFileName);
        ...
        try {
            ...
            InputStream reader = JacobDllLoader.class.getResourceAsStream(nativeLibraryFilePath);
            ...
            {
                InputStream nativeIn = JacobDllLoader.class.getResourceAsStream(nativeLibraryFilePath);
     ...
     * @throws
     */
    private static void loadJacobNativeLibrary() throws Exception {
        if(extracted) {
            return;
        }

        // Try loading library from org.sqlite.lib.path library path */
        String jacobNativeLibraryPath = System.getProperty("com.jacob.lib.path");
        String jacobNativeLibraryName = System.getProperty("com.jacob.lib.name");
        if(jacobNativeLibraryName == null) {
            jacobNativeLibraryName = System.mapLibraryName("jacob-" + getVersion() + "-" + OSInfo.getArchName());
            if(jacobNativeLibraryName != null && jacobNativeLibraryName.endsWith("dylib")) {
                jacobNativeLibraryName = jacobNativeLibraryName.replace("dylib", "jnilib");
            }
        }

        if(jacobNativeLibraryPath != null) {
            if(loadNativeLibrary(jacobNativeLibraryPath, jacobNativeLibraryName)) {
                extracted = true;
                return;
            }
        }
        ...
        String packagePath = JacobDllLoader.class.getPackage().getName().replaceAll("\\.", "/");
        jacobNativeLibraryPath = String.format("/%s/native/%s", packagePath, OSInfo.getNativeLibFolderPathForCurrentOS());
        boolean hasNativeLib = hasResource(jacobNativeLibraryPath + "/" + jacobNativeLibraryName);

        if(!hasNativeLib) {
            extracted = false;
            throw new Exception(String.format("No native library is found for os.name=%s and os.arch=%s. path=%s", OSInfo.getOSName(), OSInfo.getArchName(), jacobNativeLibraryPath));
        }

        // temporary library folder
        String tempFolder = getTempDir().getAbsolutePath();
        // Try extracting the library from jar
        if(extractAndLoadLibraryFile(jacobNativeLibraryPath, jacobNativeLibraryName, tempFolder)) {
            extracted = true;
            return;
        }
    ...
    private static boolean hasResource(String path) {
        return JacobDllLoader.class.getResource(path) != null;
    }
    ...
    public static String getVersion() {
        ...
        URL versionFile = JacobDllLoader.class.getResource("/META-INF/JacobVersion.properties");
